...
typedef struct {
    ...
    /*num_pages associated with pager object,not table since it's the
    number of pages used by the database, not a particular table.*/
    uint32_t num_pages;
} Pager;
...
Cursor* table_end(Table* table);   // Missing in original code, assumed
...
MetaCommandResult do_meta_command(InputBuffer* input_buffer, Table* table) {
    if (strcmp(input_buffer->buffer, ".exit") == 0) {
        ...
        return META_UNRECOGNIZE_COMMAND;
    }
}
...
/*** Cursor ***/
/*Represents the current position in the table (tree structure now)*/
void* cursor_value(Cursor* cursor) {
...
Cursor* table_start(Table* table) {
    /*Old implementation was flawed, because execute_select() begins at the start of the table
    our current implementation of table_start() returns cell 0 of the root node. But the root 
    of our tree is now an internal node which doesn't contain any rows. 
    The data that was printed must have been left over from when the root node was aleaf.*/
...
/* Return pos of given key, if key not present, return the position where it should be inserted*/
Cursor* table_find(Table* table, uint32_t key) {
...
Cursor* leaf_node_find(Table* table, uint32_t page_num, uint32_t key){
    /*Either returns
     1.)Position of the key
     2.)Position of another key that we'll need to move if we want to insert the new key
     3.)position one past the last key*/
...
Table* db_open(const char* filename) {
    ...
    if(pager->num_pages == 0){
        //New database file, init page 0 as the leaf node
...
void leaf_node_insert(Cursor* cursor, uint32_t key, Row* value){
    ...
    if(num_cells >= LEAF_NODE_MAX_CELLS){
        //Node is full, will need to split and increase depth
    ...
    if(cursor->cell_num < num_cells){
        ...
        for(uint32_t i = num_cells; i>cursor->cell_num; i--){
            //Equiv, leaf_node_cell[i] = leaf_node_cell[i+1] but backwards 
            //memcpy(void *dest str, const void* src_str, size_t n)l
...
/***  Leaf Node Fields ***/
//Return pointer to the value, used as getter and setter
uint32_t* leaf_node_num_cells(void* node){
...
void create_new_root(Table* table, uint32_t right_child_page_num){
    //Need to start recording each reference to the parent.
    /*
...
void internal_node_split_and_insert(Table* table, uint32_t parent_page_num, uint32_t child_page_num){
    /*The first thing we need to do is create a variable to store the page number of the node we are splitting (the old node from here out). 
    This is necessary because the page number of the old node will change if it happens to be the table’s root node.
     We also need to remember what the node’s current max is, because that value represents its key in the parent, and that key will need to be 
     updated with the old node’s new maximum after the split occurs*/
    ...
    */
    ...
    /*Once we have settled the question of splitting or not splitting the root, 
    we begin moving keys from the old node to its sibling. We must first move the old node’s 
    right child and set its right child field to an invalid page to indicate that it is empty.
     Now, we loop over the old node’s remaining keys, performing the following steps on each iteration:
    ...
    *internal_node_right_child(old_node) = INVALID_PAGE_NUM;
    ...
    /*Step 4 is important, because it serves the purpose of “erasing” the key/child pair from the old node. 
    Although we are not actually freeing the memory at that byte offset in the old node’s page, by decrementing the old node’s
     number of keys we are making that memory location inaccessible, and the bytes will be overwritten the next time a child is 
     inserted into the old node.

    Also note the behavior of our loop invariant - if our maximum number of internal node keys changes in the future, our logic ensures that 
    both our old node and our sibling node will end up with (n-1)/2 keys after the split, with the 1 remaining node going to the parent.
     If an even number is chosen as the maximum number of nodes, n/2 nodes will remain with the old node while (n-1)/2 will be moved to the sibling node. 
     This logic would be straightforward to revise as needed.
